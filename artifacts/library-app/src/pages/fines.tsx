...
import { CheckCircle2, Info, Plus, ChevronsUpDown, Check, Calculator, IndianRupee, User, GraduationCap } from "lucide-react";
import { cn } from "@/lib/utils";

type PendingPayment = { id: number; borrowerName: string; bookTitle: string; amount: number };
...
export default function Fines() {
  ...
  const confirmPayment = () => {
    ...
    clearFine.mutate({ id: pendingPayment.id }, {
      onSuccess: () => {
        toast({ title: "Fine marked as paid", description: `₹${pendingPayment.amount.toFixed(2)} collected from ${pendingPayment.borrowerName}.` });
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage and collect overdue payments from students and teachers.</p>
        </div>
      ...
      <div className="flex items-start gap-3 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">
        ...
        <div>
          ...
          Use <span className="font-semibold">Collect Fine</span> to record an on-spot payment for students or teachers.
        </div>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              fines?.map((fine) => {
                const borrowerName = fine.borrowerName ?? fine.studentName ?? fine.teacherName ?? "Unknown";
                const isTeacher = fine.borrowerType === "teacher";
                const profileHref = isTeacher
                  ? `/teachers`
                  : `/students/${fine.studentId}`;
                return (
                  <TableRow key={fine.id}>
                    <TableCell className="font-medium">
                      <div className="flex items-center gap-1.5">
                        ...
                        <Link href={profileHref} className="hover:underline">{borrowerName}</Link>
                    ...
                    <TableCell>
                      {fine.loanId ? (
                        <span className="truncate max-w-[200px] block" title={fine.bookTitle || ''}>
                          {fine.bookTitle}
                        </span>
                      ) : (
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-3">
                <p>
                  ...
                  <span className="font-semibold text-foreground">{pendingPayment?.borrowerName}</span>?
                </p>
...
type BorrowerType = "student" | "teacher";
...
function CollectFineDialog({
  ...
  const { data: teachers } = useQuery<{ id: number; name: string; teacherId: string }[]>({
    queryKey: ["teachers"],
    queryFn: async () => {
  ...
  const selectedBorrowerName = borrowerType === "teacher"
    ? selectedTeacher ? `${selectedTeacher.name} (${selectedTeacher.teacherId})` : null
    : selectedStudent ? `${selectedStudent.name} (${selectedStudent.studentId})` : null;
  ...
  const handleSubmit = async () => {
    const hasBorrower = borrowerType === "teacher" ? !!selectedTeacherId : !!selectedStudentId;
    if (!hasBorrower) {
      toast({ title: `Select a ${borrowerType}`, variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(resBody.error ?? "Failed to record fine");
      }
      toast({
        title: "Fine collected",
        description: `₹${computedAmount.toFixed(2)} collected from ${selectedBorrowerName} and recorded.`,
      });
      onSuccess();
      handleClose();
    } catch (err: any) {
  ...
  const canSubmit = (borrowerType === "teacher" ? !!selectedTeacherId : !!selectedStudentId)
    && computedAmount > 0 && !!reason && !submitting;
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) handleClose(); }}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Record a cash payment collected directly from a student or teacher. The fine will be marked as paid immediately.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 pt-1">
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button
                ...
                size="sm"
                className="w-full"
                onClick={() => { setBorrowerType("student"); setSelectedTeacherId(null); setBorrowerSearch(""); }}
              >
                <User className="h-3.5 w-3.5 mr-1.5" /> Student
              </Button>
              <Button
                type="button"
                variant={borrowerType === "teacher" ? "default" : "outline"}
                size="sm"
                className="w-full"
                onClick={() => { setBorrowerType("teacher"); setSelectedStudentId(null); setBorrowerSearch(""); }}
              >
                <GraduationCap className="h-3.5 w-3.5 mr-1.5" /> Teacher
              </Button>
            </div>
          </div>
          ...
          {/* Borrower picker */}
          <div className="space-y-1.5">
            <Label>{borrowerType === "teacher" ? "Teacher" : "Student"}</Label>
            <Popover open={borrowerPopoverOpen} onOpenChange={setBorrowerPopoverOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  {selectedBorrowerName ?? `Search ${borrowerType} by name or ID…`}
              ...
              <PopoverContent className="w-full p-0" align="start">
                <Command shouldFilter={false}>
                  <CommandInput
                    placeholder={`Type name or ${borrowerType} ID…`}
                    value={borrowerSearch}
                    onValueChange={setBorrowerSearch}
                  />
                  <CommandList>
                    <CommandEmpty>No {borrowerType}s found.</CommandEmpty>
                    <CommandGroup>
          ...
          {/* Summary */}
          {selectedBorrowerName && computedAmount > 0 && (
            <div className="rounded-md border bg-emerald-50 border-emerald-200 px-4 py-3 space-y-1 text-sm">
              ...
              <p className="text-emerald-700 font-medium">{selectedBorrowerName}</p>
